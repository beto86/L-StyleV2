    ...
    if pdf.err:
        ...
        for entry in pdf.log:
            if entry[0] == xhtml2pdf.default.PML_ERROR:
                logger_x2p.error("line %s, msg: %s, fragment: %s",
                                 entry[1], entry[2], entry[3])
        raise PDFRenderingError("Errors rendering PDF",
                                content=content, log=pdf.log)

    if pdf.warn:
        for entry in pdf.log:
            if entry[0] == xhtml2pdf.default.PML_WARNING:
                logger_x2p.warning(
                    "line %s, msg: %s, fragment: %s", entry[1], entry[2], entry[3])
...
def make_response(content, filename=None, content_type="application/pdf"):
    ...
    if filename is not None:
        response["Content-Disposition"] = "attachment; %s" % encode_filename(
            filename)
    return response
...
def render_to_pdf(template, context, using=None, request=None, encoding="utf-8", **kwargs):
    ...
    content = loader.render_to_string(
        template, context, request=request, using=using)
...
def render_to_pdf_response(request, template, context, using=None, filename=None,
    ...
    """
    try:
        pdf = render_to_pdf(template, context, using=using,
